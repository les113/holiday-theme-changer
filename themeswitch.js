	jQuery(document).ready(function($){ 
		...
		//var mm = 9; // alert(mm);	//test		
		//var dd = 31; // alert(dd);	//test

		// select holiday theme
		var theme;		
		var message;
		if (dd == 31 && mm == 11) {theme = 9; message="Happy New Year!";} // it's new years eve
		else if(dd == 1 && mm == 0) {theme = 9; message="Happy New Year!";} // it's new years day
		else if(dd == 14 && mm == 1) {theme = 7; message = "Happy Valentines Day";} // it's valentines day
		else if(dd >= 18 && dd <= 21 && mm == 3) {theme = 8; message = "Happy Easter!";} // it's easter
		else if(dd == 5 && mm == 10) {theme = 9;} // it's bonfirenight 
		else if(dd == 31 && mm == 9) {theme = 10; message ="Happy Halloween";} // it's halloween
		else if(dd >=18 && dd <=23 && mm == 11) {theme = 11; message="We wish you a Merry Christmas!";		
									//$.fn.snow({ maxSize: 30, newOn: 250}); snow not working in IE arrgh!
									} // it's nearly xmas!
		else if(dd >=24 && dd <=30 && mm == 11) {theme = 12; message="Happy Christmas!";		
									//$.fn.snow({ maxSize: 30, newOn: 100 });
									} // it's xmas day
									
		 else wintheme(); //calls win8 theme if not a holiday
